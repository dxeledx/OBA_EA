    ...
    *,
    ...
    if cand.empty:
        return {
            ...
            "delta_acc": 0.0,
            "oracle_acc": float(anchor_acc) if np.isfinite(anchor_acc) else float("nan"),
        }
    ...
    if float(max(best_pred, best_probe)) <= 0.0 and np.isfinite(anchor_acc):
        return {
            ...
            "anchor_acc": float(anchor_acc),
            "delta_acc": 0.0,
            "oracle_acc": float(anchor_acc),
        }

    sel_acc = _safe_float(best.get("accuracy", float("nan")))
    fam = str(best.get("cand_family", "other"))

    # Oracle over the *available candidate set* (for gap decomposition).
    oracle_acc = float(cand_df["accuracy"].astype(float).max())
